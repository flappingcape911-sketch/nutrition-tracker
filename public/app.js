...
function cacheDom() {
  refs.authCard = document.querySelector("#auth-card");
  refs.dashboard = document.querySelector("#dashboard");
  refs.authForm = document.querySelector("#auth-form");
  refs.authEmail = document.querySelector("#auth-email");
  refs.authPassword = document.querySelector("#auth-password");
  refs.authSetupCode = document.querySelector("#auth-setup-code");
  refs.authStatus = document.querySelector("#auth-status");
  refs.authSubmitBtn = document.querySelector("#auth-submit-btn");
  refs.logoutBtn = document.querySelector("#logout-btn");
  refs.authTitle = document.querySelector("#auth-title");
  refs.authCaption = document.querySelector("#auth-caption");
  refs.setupCodeLabel = document.querySelector("#setup-code-label");
...
async function hydrateState() {
  state.auth = await loadSession();

  if (state.auth.authenticated) {
    const accountState = await loadAccountState();
    const migratedState = await maybeImportLocalState(accountState);
    state.profile = migratedState.profile;
    state.foods = mergeFoods(migratedState.customFoods);
...
function initializeFormDefaults() {
  ...
  populateProfileForm();
  setProfileStatus("Your goals and AI preferences will sync to your account.", "");
  setAuthStatus("Create your private account once, then sign in from any device to load the same data.", "");
}
...
function renderAuthCard() {
  ...
  refs.authTitle.textContent = signedIn
    ? "Signed in to your nutrition account"
    : state.auth.registrationOpen
      ? "Create your private nutrition account"
      : "Sign in to your nutrition account";
  refs.authCaption.textContent = signedIn
    ? `Signed in as ${state.auth.user?.email || "your account"}. Your meals, goals, and foods will sync on every device.`
    : state.auth.registrationOpen
      ? "Create the one private owner account for this tracker, then use the same login anywhere."
      : "Sign in to load the same meals, goals, and custom foods on any device.";
  refs.setupCodeLabel.classList.toggle("hidden", signedIn || !state.auth.registrationOpen);
  refs.authSubmitBtn.classList.toggle("hidden", signedIn);
  refs.logoutBtn.classList.toggle("hidden", !signedIn);
  refs.authSubmitBtn.textContent = state.auth.registrationOpen ? "Create account" : "Sign in";
  refs.authEmail.disabled = signedIn;
  refs.authPassword.disabled = signedIn;
  refs.authSetupCode.disabled = signedIn || !state.auth.registrationOpen;

  if (signedIn) {
    refs.authEmail.value = state.auth.user?.email || "";
    refs.authPassword.value = "";
    refs.authSetupCode.value = "";
  }
}
...
async function loadSession() {
  try {
    ...
    return {
      ...
      user: payload.user || (payload.email ? { email: payload.email } : null),
    };
  } catch (error) {
    setAuthStatus(error.message || "Unable to load your sign-in status.", "error");
    return {
      authenticated: false,
      registrationOpen: false,
      user: null,
...
async function loadAccountState() {
  ...
  const payload = await response.json();
  if (!response.ok) {
    throw new Error(payload.error || "Unable to load your account state.");
  }
  return {
...
async function maybeImportLocalState(accountState) {
  const localProfile = loadProfile();
  const localFoods = loadFoods().filter((food) => !food.builtIn);
  const shouldImportProfile = !hasMeaningfulProfile(accountState.profile) && hasMeaningfulProfile(localProfile);
  const shouldImportFoods = !accountState.customFoods.length && localFoods.length > 0;

  if (!shouldImportProfile && !shouldImportFoods) {
    persistJson(STORAGE_KEYS.profile, accountState.profile);
    persistJson(STORAGE_KEYS.foods, mergeFoods(accountState.customFoods));
    return accountState;
  }

  const mergedProfile = shouldImportProfile ? localProfile : accountState.profile;
  const mergedFoods = shouldImportFoods ? localFoods : accountState.customFoods;
  await saveAccountStateToServer(mergedProfile, mergedFoods);
  return {
    profile: mergedProfile,
    customFoods: mergedFoods,
  };
}
...
function mergeFoods(customFoods) {
  const mergedFoods = clone(BUILT_IN_FOODS);
  (Array.isArray(customFoods) ? customFoods : []).forEach((food) => {
    if (!mergedFoods.some((item) => item.id === food.id)) {
      mergedFoods.unshift(food);
    }
  });
  return mergedFoods;
}
...
function getCustomFoods() {
  return state.foods.filter((food) => !food.builtIn);
}
...
async function handleAuthSubmit(event) {
  ...
  const setupCode = refs.authSetupCode.value.trim();
  const endpoint = state.auth.registrationOpen ? "/api/auth/register" : "/api/auth/login";

  setAuthStatus(state.auth.registrationOpen ? "Creating your private account..." : "Signing you in...", "");
  ...
  try {
    ...
    populateProfileForm();
    resetMealComposer();
    setAuthStatus(`Signed in as ${state.auth.user?.email || email}.`, "success");
    renderAll();
    await checkAiAvailability();
    await checkStorageAvailability();
  } catch (error) {
    setAuthStatus(error.message || "Unable to complete sign-in.", "error");
  }
}
...
async function handleLogout() {
  try {
    ...
    // Fall back to local signed-out state even if the network request fails.
  }
  ...
  populateProfileForm();
  resetMealComposer();
  setAuthStatus("Signed out. Sign in again to open your private nutrition account.", "");
  setStorageStatus("Sign in to sync your meals, profile, and custom foods across devices.", "");
  setAiStatus("Sign in to analyze meals with your private account.", "");
  renderAll();
}
...
function ensureAuthenticatedAction() {
  ...
  setAuthStatus("Sign in first so the tracker can load and save your private cloud data.", "error");
  refs.authEmail.focus();
  return false;
}

async function saveAccountStateToServer(profile, customFoods) {
  const response = await fetch("/api/account-state", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
    body: JSON.stringify({ profile, customFoods }),
  });
  const payload = await response.json();
  if (!response.ok) {
    throw new Error(payload.error || "Unable to save your account state.");
  }
  persistJson(STORAGE_KEYS.profile, profile);
  persistJson(STORAGE_KEYS.foods, mergeFoods(customFoods));
  return payload;
}
...
function renderDailyStats(totals, mealCount) {
  ...
  const calorieDelta = calorieGoal - totals.calories;
  const stats = [
    {
      label: "Calories",
    ...
    },
    {
      ...
      value: `${formatNumber(Math.abs(calorieDelta), 0)} kcal`,
      subvalue: calorieDelta >= 0 ? "Below your goal today" : "Above your goal today",
    },
    {
      label: "Protein",
...
async function saveMeal(event) {
  ...
  try {
    ...
    setStorageStatus(error.message || "Unable to save this meal to cloud storage.", "error");
    return;
  }

  state.logs[state.currentDate].push(meal);
  persistLogs();
  setStorageStatus("Meal saved to cloud storage.", "success");
  resetMealComposer();
  renderAll();
}
...
async function saveCustomFood(event) {
  ...
  try {
    await saveAccountStateToServer(state.profile, getCustomFoods());
    setProfileStatus("Custom food saved to your cloud account.", "success");
  } catch (error) {
    setProfileStatus(error.message || "Unable to save that food to your account.", "error");
  }
...
async function saveProfileSettings(event) {
  ...
  try {
    await saveAccountStateToServer(state.profile, getCustomFoods());
    setProfileStatus("Profile settings saved to your cloud account.", "success");
  } catch (error) {
    setProfileStatus(error.message || "Unable to save profile settings right now.", "error");
  }

  renderAll();
}

async function saveInlineCustomFood(event) {
  if (!ensureAuthenticatedAction()) {
    return;
  }
...
async function checkAiAvailability() {
  ...
  try {
    ...
    if (response.ok && payload.geminiConfigured) {
      setAiStatus(`AI analysis ready in the cloud using Gemini ${payload.geminiModel}.`, "success");
    } else {
      setAiStatus("Gemini is not configured in Cloudflare yet.", "error");
    }
  } catch (error) {
    setAiStatus("AI analysis is not reachable right now. Check your Cloudflare Functions deployment and Gemini secret.", "error");
  }
}
...
async function checkStorageAvailability() {
  ...
  try {
    const response = await fetch("/api/health");
    const payload = await response.json();
    if (response.ok && payload.d1Ready) {
      setStorageStatus("Meals, goals, and custom foods are syncing to your personal cloud account.", "success");
      return;
    }

    if (response.ok && payload.d1Bound) {
      setStorageStatus(payload.d1Error ? `Cloudflare D1 is bound, but the database is not ready yet. ${payload.d1Error}` : "Cloudflare D1 is bound, but the database is not ready yet.", "error");
      return;
    }

    setStorageStatus("Cloudflare D1 is not configured yet. The app is using browser backup storage until cloud storage is ready.", "error");
  } catch (error) {
    setStorageStatus("The cloud API is not reachable, so meal logs are only in browser backup storage right now.", "error");
  }
}
...
async function handleMealLogClick(event) {
  ...
  try {
    ...
    setStorageStatus(error.message || "Unable to delete this meal from cloud storage.", "error");
    return;
  }
  ...
  persistLogs();
  setStorageStatus("Meal deleted from cloud storage.", "success");
  renderAll();
}
...
function buildDayAnalysis(totals, meals) {
  ...
  if (calorieGoal) {
    ...
    summary.push(
      calorieDelta >= 0
        ? `You are ${formatNumber(calorieDelta, 0)} kcal below your calorie goal today.`
        : `You are ${formatNumber(Math.abs(calorieDelta), 0)} kcal above your calorie goal today.`
    );
  }
...
function loadFoods() {
  ...
  const customFoods = Array.isArray(storedFoods) ? storedFoods.filter((food) => food && !food.builtIn) : [];
  const mergedFoods = mergeFoods(customFoods);
...
async function loadLogs() {
  ...
  try {
    ...
    setStorageStatus("Using browser backup logs because cloud storage is not reachable yet.", "error");
    return fallbackLogs;
  }
}
...
function setAuthStatus(message, variant) {
  ...
  refs.authStatus.className = variant ? `helper-text ${variant}` : "helper-text";
}
...
function resetMealComposer() {
  ...
  refs.servingsInput.value = "1";
  refs.mealTime.value = currentTime();
  setAiStatus(state.auth.authenticated ? "AI analysis uses your Gemini key securely in Cloudflare." : "Sign in to analyze meals with your private account.", "");
  renderAiControls();
  renderCurrentMeal();
}
